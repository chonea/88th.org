 ...
 * @version    $Id: class.ts3viewer.php 45 2012-10-10 10:35:44Z branko.wilhelm@gmail.com $
 */
class TS3Viewer {
     ...
     */
    //public $cache_path = '/sites/default/files/cache/ts3';
    public $cache_path = '';
    ...
    public function __construct($host=null, $query=null, $sid=null, $port=null) {
        ...
        $this->lang['stats']['uptime_val'] = '%d Days %02d Hours %02d Min';
    ...
    private function refresh() {
        ...
        // try & catch wär zwar schöner, funktioniert nur bei fsockopen nicht ..
        $this->socket = @fsockopen($this->ts3_host, $this->ts3_query, $errno, $errstr, $this->socket_timeout);

        if ($errno > 0 && $errstr != '') {
            $this->errors[] = 'fsockopen error: ' . $errstr . ' (' . $errno . ')';
            return false;
        }
    ...
    private function uptime($sec) {
        ...
        return sprintf($this->lang['stats']['uptime_val'], $sec / 60 / 60 / 24, ($sec / 60 / 60) % 24, ($sec / 60) % 60);
    }
